...
Namespace PartProcess
    Public Class ClearButtonProcess
        ...
        Public Shared Function ClearAllRows(ByVal tableName As String) As ProcessResult
            Dim accessor As SQLiteAccessor = DbAccessManager.GetSQLiteAccessor(FileUtility.DbFilePath)

            Try
                Using manager As New TransactionManager(accessor)
                    Dim deleteCommand As New SqlDeleteCommand(tableName)
                    accessor.ExecuteCommand(deleteCommand)
                    manager.Commit()
                End Using
                Return ProcessResult.Success
            Catch ex As DbAccessException
